...
public class DailyQuestSplineInternal
{
	...
	public event DailyQuestSplineInternal.DirtyHandler WasMadeDirty;
	...
	public void CalculateSegments(int segmentsCount)
	{
		...
		this.segments = new List<DailyQuestSplineInternal.Segment>();
		this.segmentsLength = 0f;
		float num2 = 0f;
		for (int i = 0; i < segmentsCount; i++)
		{
			DailyQuestSplineInternal.Segment segment = new DailyQuestSplineInternal.Segment(this.Evaluate(num2), this.Evaluate(num2 + num), num2, num2 + num);
	...
	{
		float t;
		DailyQuestSplineInternal.Segment segment = this.FindSegment(position, out t);
		return Vector2.Lerp(segment.from, segment.to, t);
	}

	private DailyQuestSplineInternal.Segment FindSegment(float position, out float amount)
	{
		if (position < 0f)
		{
			amount = 0f;
		...
		{
			DailyQuestSplineInternal.Segment result = this.segments[this.segments.Count - 1];
			amount = 1f;
			return result;
		}
		amount = 0f;
	...
	public void CalculateSpline()
	{
		...
		this.curves = new List<DailyQuestSplineInternal.Curve>();
		for (int i = 0; i < num; i++)
		{
			this.curves.Add(new DailyQuestSplineInternal.Curve());
		}
		this.fullLength = 0f;
		if (!this.Cyclic)
		{
			for (int j = 0; j < num; j++)
			{
				...
				DailyQuestSplineInternal.Curve curve = this.curves[j];
		...
		else
		{
			for (int k = 0; k < num; k++)
			{
				...
				DailyQuestSplineInternal.Curve curve2 = this.curves[k];
	...
	private DailyQuestSplineInternal.Curve FindCurve(float position, out float amount, out int curveIndex)
	{
		if (position < 0f)
		{
			amount = 0f;
		...
		{
			DailyQuestSplineInternal.Curve result = this.curves[this.curves.Count - 1];
			amount = 1f;
	...
	public Vector2 Evaluate(float position)
	{
		...
		float mu;
		int num;
		DailyQuestSplineInternal.Curve curve = this.FindCurve(position, out mu, out num);
	...
	public void EvaluateTwoPositions(float position, out Vector2 pos1, out Vector2 pos2)
	{
		...
		float num;
		int num2;
		DailyQuestSplineInternal.Curve curve = this.FindCurve(position, out num, out num2);
	...
	public DailyQuestSplineInternal.SplineInfo EvaluateWithInfo(float position)
	{
		this.CalculateSpline();
		DailyQuestSplineInternal.SplineInfo result = default(DailyQuestSplineInternal.SplineInfo);
		if (this.curves == null || this.curves.Count < 1)
		{
			return result;
		}
		float num;
		int curveIndex;
		DailyQuestSplineInternal.Curve curve = this.FindCurve(position, out num, out curveIndex);
		result.alpha = num;
		result.curve = curve;
	...
	[SerializeField]
	public List<DailyQuestSplineInternal.Curve> curves;
	...
	private List<DailyQuestSplineInternal.Segment> segments;

	private float segmentsLength;

	public delegate void DirtyHandler(DailyQuestSplineInternal spline);
	...
	public struct SplineInfo
	{
		...
		public DailyQuestSplineInternal.Curve curve;
