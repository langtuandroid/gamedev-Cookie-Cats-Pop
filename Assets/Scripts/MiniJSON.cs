...
public class MiniJSON
{
	...
	public static string jsonEncode(object json, bool prettyPrint = false, int indent = 0)
	{
		StringBuilder stringBuilder = new StringBuilder(2000);
		bool flag = MiniJSON.serializeValue(json, stringBuilder, prettyPrint, indent, 0);
	...
	protected static bool serializeObjectOrArray(object objectOrArray, StringBuilder builder)
	{
		...
		{
			return MiniJSON.serializeObject((Hashtable)objectOrArray, builder, false, 0, 0);
		}
		return objectOrArray is ArrayList && MiniJSON.serializeArray((ArrayList)objectOrArray, builder, false, 0, 0);
	}

	protected static bool serializeObject(Hashtable anObject, StringBuilder builder, bool prettyPrint = false, int indent = 0, int indentLevel = 0)
	{
		builder.Append("{");
		...
		while (enumerator.MoveNext())
		{
			...
			if (prettyPrint)
			{
				...
				builder.Append("\n");
				builder.Append(' ', indentLevel * indent);
			}
			MiniJSON.serializeString(aString, builder);
			builder.Append((!prettyPrint) ? ":" : ": ");
			if (!MiniJSON.serializeValue(value, builder, prettyPrint, indent, indentLevel))
			{
				return false;
			}
			flag = false;
	...
	protected static bool serializeDictionary(Dictionary<string, string> dict, StringBuilder builder, bool prettyPrint = false, int indent = 0, int indentLevel = 0)
	{
		builder.Append("{");
		bool flag = true;
	...
	protected static bool serializeArray(ArrayList anArray, StringBuilder builder, bool prettyPrint = false, int indent = 0, int indentLevel = 0)
	{
		builder.Append("[");
		bool flag = true;
		...
		if (prettyPrint && anArray.Count > 0)
		{
			indentLevel--;
			builder.Append("\n");
			builder.Append(' ', indentLevel * indent);
	...
	protected static bool serializeValue(object value, StringBuilder builder, bool prettyPrint = false, int indent = 0, int indentLevel = 0)
	{
		if (value == null)
		{
			builder.Append("null");
		...
		{
			MiniJSON.serializeArray(new ArrayList((ICollection)value), builder, prettyPrint, indent, indentLevel);
		}
		...
		{
			MiniJSON.serializeObject((Hashtable)value, builder, prettyPrint, indent, indentLevel);
		}
		else if (value is Dictionary<string, string>)
		{
			MiniJSON.serializeDictionary((Dictionary<string, string>)value, builder, prettyPrint, indent, indentLevel);
		}
		else if (value is ArrayList)
		{
			MiniJSON.serializeArray((ArrayList)value, builder, prettyPrint, indent, indentLevel);
		}
		...
		else
		{
			...
			{
				UnityEngine.Debug.LogError("MiniJSON - Could not serialize object of type: " + value.GetType() + ". Consider converting the object to a hashtable.");
				return false;
			}
			if (value.GetType() == typeof(long))
			{
				MiniJSON.serializeNumber((long)value, builder);
			}
			else
			{
	...
	protected static void serializeNumber(long number, StringBuilder builder)
	{
