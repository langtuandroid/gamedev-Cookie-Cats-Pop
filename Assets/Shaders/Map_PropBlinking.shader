Shader "Map/PropBlinking"
{
  Properties
  {
    [PerRendererData] _MainTex ("Base (RGB), Alpha (A)", 2D) = "white" {}
    [PerRendererData] _AlphaTex ("Base (RGB), Alpha (A)", 2D) = "white" {}
  }
  SubShader
  {
    Tags
    ...
    Pass // ind: 1, name: 
    {
      ...
      // m_ProgramMask = 6
      CGPROGRAM
      #pragma multi_compile ALPHA_SPLITTING_OFF
      //#pragma target 4.0
      ...
      OUT_Data_Frag frag(v2f in_f)
      {
          ...
          float tmpvar_3;
          tmpvar_3 = tex2D(_LightTexture, in_f.xlv_TEXCOORD1).x;
          float4 y_4;
          y_4 = (tmpvar_2 * _NightColor);
          float3 tmpvar_5;
          tmpvar_5 = lerp(lerp(tmpvar_2, y_4, float4(_NightBlend, _NightBlend, _NightBlend, _NightBlend)), tmpvar_2, float4(tmpvar_3, tmpvar_3, tmpvar_3, tmpvar_3)).xyz;
          col_1 = tmpvar_5;
          col_1 = (col_1 + float3(_GlowStrength, _GlowStrength, _GlowStrength));
          float4 tmpvar_6;
